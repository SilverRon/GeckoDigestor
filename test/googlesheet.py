...
class GoogleSheet:
    ...
    def __init__(self,
                 ...
                 authorize_json_file : str = './Google_authorize_lmd13bnd.json',
                 scope = [
...
#     # Make GoogleSheet instance
#     gs = GoogleSheet((GoogleSheet().address_sheets['targetlist']))
#     # Get the target format for ToO & write target(s) with the proper data type
...
#     # Set default value for each column 
...
format_tbl['weight'] = format_tbl['weight'].astype('float')
# format_tbl['note'] = format_tbl['note'].astype('int')
format_tbl['note'] = format_tbl['note'].astype('str')

#	Unifying all column types to strings
common_colnames_list = list(common_colnames)
from astropy.table import Column
for colname in common_colnames_list:
    format_tbl[colname] = Column(format_tbl[colname].data.astype(str), name=colname)
    tbl[colname] = Column(tbl[colname].data.astype(str), name=colname)
