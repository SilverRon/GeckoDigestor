...
PROCESSING_REVERSE_ORDER = config['PROCESSING_REVERSE_ORDER']
...
MAX_MEAN_DISTANCE = event_config['DISTANCE']['MAX_MEAN']
IGNORE_BBH = event_config['EVENT']['IGNORE_BBH']
IGNORE_FAR_BBH = event_config['EVENT']['IGNORE_FAR_BBH']
IGNORE_TERRESTRIAL = event_config['EVENT']['IGNORE_TERRESTRIAL']
IGNORE_BURST = event_config['EVENT']['IGNORE_BURST']
...
print("\nAPI Settings:")
print(f"  GCN Kafka Client ID: {client_id}")
...
print("\nTime Settings:")
print(f"  Time to Sleep: {TIME_TO_SLEEP} seconds")

print("\nVisualization Settings:")
print(f"  Plot DPI: {plot_config['DPI']}")
print(f"  Title Size: {plot_config['TITLE_SIZE']}")
print(f"  Label Size: {plot_config['LABEL_SIZE']}")

print("\nEvent Processing Settings:")
print(f"  Distance Thresholds: {DISTANCE_THRESHOLDS}")
print(f"  Max Mean Distance: {MAX_MEAN_DISTANCE} Mpc")
print(f"  Ignore BBH: {IGNORE_BBH}")
print(f"  Ignore Far BBH: {IGNORE_FAR_BBH}")
print(f"  Ignore Terrestrial: {IGNORE_TERRESTRIAL}")
print(f"  Ignore Burst: {IGNORE_BURST}")
print(f"  Probability Thresholds: {PROBABILITY_THRESHOLDS}")
print(f"  FARCUT: {FARCUT}")

print("\nDebug Settings:")
print(f"  Debug Mode: {debug_mode}")
print(f"  Processing Reverse Order: {PROCESSING_REVERSE_ORDER}")
...
while True:	
	...
	if new_events:
		...
		if PROCESSING_REVERSE_ORDER:
			...
			print("Processing in forward order")
	...
	# for ee, event in enumerate(new_events):
		...
		# %%
		# 	Apply Configured Setting for Gecko Digestor Que
		if (most_probable_event == "BBH") & (record['distmean'] > MAX_MEAN_DISTANCE) & (IGNORE_FAR_BBH):
			gecko_digestor_trigger = False
			slack = False
			print(f"Ignore far BBH event --> gecko_digestor_trigger: {gecko_digestor_trigger}, slack: {slack}")
		if (most_probable_event == "Terrestrial") & (IGNORE_TERRESTRIAL):
			gecko_digestor_trigger = False
			slack = False
			print(f"Ignore terrestrial event --> gecko_digestor_trigger: {gecko_digestor_trigger}, slack: {slack}")
		if (most_probable_event == "BBH") & (IGNORE_BBH):
			gecko_digestor_trigger = False
			slack = False
			print(f"Ignore BBH event --> gecko_digestor_trigger: {gecko_digestor_trigger}, slack: {slack}")
		if (most_probable_event == "Burst") & (IGNORE_BURST):
			gecko_digestor_trigger = False
			slack = False
		...
		def cleanup_memory():
			...
			"""
			var_names = ['skymap', 'select_cat', 'simple_galcat', 'record', 'result']
			global_vars = globals()
			for name in var_names:
				if name in global_vars and global_vars[name] is not None:
