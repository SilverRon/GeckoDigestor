...
class ObsScheduler(mainConfig):
    ...
    def set_obs_info(self, name_telescope):
        ...
        sunset_prepare, sunrise_prepare, sunset_astro, sunrise_astro, obs_start, obs_end = self._set_obs_night()
        self.ast_allnight = (sunset_astro, sunrise_astro)
        self.civil_allnight = (sunset_prepare, sunrise_prepare)
        self.midnight = self._get_midnight()
        self.obs_night = (obs_start, obs_end)
        self.moon_radec = self._get_moon_radec(self.midnight)

        target_coords = self._match_coord_format(self.target_all['ra'].tolist(), self.target_all['dec'].tolist())
        ...
        self.target_all['moonsep'] = moonsep
        self.target_observable = self._get_target_observable(0.15)
    ...
    def scheduler(self):
        ...
        if len(obs_tbl) == 0:
            print('[scheduler] FAILED! No observable Target.')
        ...
        while time_observation < time_obs_end:
            ...
            if self.config['SCHEDULER_AUTOFOCUS']:
                if (time_observation - autofocus_time > self.config['SCHEDULER_AFTIME'] * u.hour) * (time_astronomical_dawn - time_observation > self.config['SCHEDULER_AFTIME'] * u.hour):
                    ...
                    autofocus_time = time_observation
            # Target selection 
            target_best, telescope_altaz, target_score = self._scorer(obs_tbl, telescope_altaz, utctime = time_observation, order=1)
            
            if target_score == 0:
                if self._duplicate:
                    target_best, telescope_altaz, target_score = self._scorer(obs_tbl, telescope_altaz, utctime = time_observation, duplicate= True, order = np.random.randint(10))
                    if target_score == 0:
    ...
    def write_Gsheet(self,
        ...
        scheduled_table = self.scheduled_table.copy()
        if len(scheduled_table) == 0:
            print('[write_Gsheet] FAILED! No observable Target.')
            return 
    ...
    def write_txt(self,
        ...
        if scheduled_only:
            ...
            if len(data_table) == 0:
                print('[write_txt] FAILED! No observable Target.')
                return 
        else:
        ...
        #filename = f'{savepath}{filename_prefix}_{self._ut_to_lt(self.midnight.datetime).strftime("%y%m%d")}_{self.name_telescope}.dat'
        # risetime_lt = []
        # transittime_lt = []
        # settime_lt = []
        ...
        print(f'[write_txt] SAVED! filename : {filename}')
    ...
    def write_rts(self,
        ...
        scheduled_table = self.scheduled_table.copy()
        if len(scheduled_table) == 0:
            print('[write_rts] FAILED! No observable Target.')
            return 
    ...
    def _get_rts(self,
        ...
        for target in all_target:
            transittime = target.meridiantime(self.midnight, n_grid_points = 40)
            if 't' in mode:
                transittimelist.append(transittime.datetime)
            if 'r' in mode:
                risetime = target.risetime(transittime, n_grid_points = 40, mode = 'previous')
                risetimelist.append(risetime.datetime)
            if 's' in mode:
                settime = target.settime(transittime, n_grid_points = 40, mode = 'next')
    ...
    def _set_obs_night(self,
        ...
        class night: pass
        time_input = Time(self.date)
        sunrise_prepare = self.observer_tcspy.tonight(time = time_input, horizon = horizon_prepare)[1]
        sunset_prepare = self.observer_tcspy.sun_settime(sunrise_prepare, mode = 'previous', horizon= horizon_prepare)
        sunrise_astro = self.observer_tcspy.sun_risetime(sunrise_prepare, mode = 'previous', horizon= horizon_astro)
        sunset_astro = self.observer_tcspy.sun_settime(sunrise_prepare, mode = 'previous', horizon= horizon_astro)
        if self._entire_night:
            obs_start = sunset_astro
        else:
            obs_start = time_input
        obs_end = sunrise_astro
        return sunset_prepare, sunrise_prepare, sunset_astro, sunrise_astro, obs_start, obs_end
...
if __name__ == '__main__':
    ...
    #data_original = ascii.read('/Users/hhchoi1022/Desktop/HostGalaxyCatalog_90.csv')
    #data_original.rename_column('name', 'obj')
    #data_original = data_original[:]
    data_original = ascii.read('./test_IMSNG.dat', format = 'fixed_width')
    data_original.rename_column('priority', 'weight')
    data_original.remove_columns(['filter','exptime','count','binning'])
    data = data_original[:]
    date = Time.now()
    ...
    obs.write_rts(filename_prefix =file_prefix, scheduled_only= True)
    ...
    #date += 1* u.day
    # import time
    # obs = ObsScheduler(target_db = data, project = project, name_telescope = 'LOAO', entire_night = True, date = date, duplicate_when_empty= False)
    ...
    # obs = ObsScheduler(target_db = data, project = project, name_teescope = 'LOAO', entire_night = True, date = date)
    # obs.write_ACPscript_KCT(filename_prefix =file_prefix, shutdown = True) # for RASA36, shutdown = False
    # obs.write_ACPscript_RASA36(filename_prefix =file_prefix) # for RASA36, shutdown = False
    # obs.write_ACPscript_LSGT(filename_prefix =file_prefix, period_script= 3) # for RASA36, shutdown = False
    # obs.write_rts(filename_prefix =file_prefix)
