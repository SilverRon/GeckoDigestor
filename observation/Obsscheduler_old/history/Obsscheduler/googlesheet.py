...
class GoogleSheet:
    ...
    def __init__(self,
                 ...
                 authorize_json_file : str = '../Config/Google_authorize_lmd13bnd.json',
                 scope = [
...
if __name__ == '__main__':
    gs = GoogleSheet()
    gs = GoogleSheet(gs.address_sheets['targetlist'])
    data_file = '/Users/hhchoi1022/Gitrepo/Config/alltarget.dat'
    data = ascii.read(data_file, format = 'fixed_width')
    #gs.write_sheet_data(sheet_name = 'IMSNG', data = data, append = False)
    tbl = gs.get_sheet_data(sheet_name = 'ToO', format_ = 'pandas')
    #gs.write_sheet_data(sheet_name= 'ToO', data = tbl, append = False, clear = True)
    #ToO_tbl = Table(names = ['obj', 'ra', 'dec', 'exptime', 'filter', 'count', 'binning', 'priority', 'update_time', 'status', 'note'])
    #gs.write_sheet_data(sheet_name = 'ToO', data = ToO_tbl, append = True, clear = True)
    all_priority = []
    for i in range(len(tbl)):
        all_priority.append(int(5000* np.random.rand()))
    tbl['priority'] = all_priority
    gs.write_sheet_data(sheet_name = 'ToO', data  =tbl, append= False, clear_header = False)
#%%
if __name__ == '__main__':
    gs = GoogleSheet()
    gs = GoogleSheet(gs.address_sheets['targetlist'])
    tbl = gs.get_sheet_data(sheet_name = 'ToO', format_ = 'Table')
    tbl.write('SampleTargetlist.ascii_fixed_width', format = 'ascii.fixed_width', overwrite = True)
# %%
