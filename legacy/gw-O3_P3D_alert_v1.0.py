...
#	2020.01.16	UPDATED BY	Gregory S.H. Paek
#============================================================
#	MODULE
#------------------------------------------------------------
import gcn
...
xmlfile = 'MS181101ab-1-Preliminary.xml'
payload		= open(path_test+'/{}'.format(xmlfile), 'rb').read()
root		= lxml.etree.fromstring(payload)

payload = open('S200213t-Update.xml', 'rb').read()
root = lxml.etree.fromstring(payload)
...
def process_gcn(payload, root):
	...
	for key in xmlkeys:
		try:
			...
			pass
		if key == 'FAR':
			f.write('1/FAR :\t1/{} yr\n'.format(round(1/float(params['FAR'])/(365*24*60*60), 2)))
	f.write('='*60+'\n')
	for key in hdrkeys:
		try:
	...
	if len(cutbl) == 0:
		cutbl = cantbl
		print('REMOVE CONFIDENCE LIMIT')
		f.write('# REMOVE CONFIDENCE LIMIT\n')
	rtstbl = Table()
	...
	if len(rtstbl) != 0:
		...
		for obs in ['SAO', 'McD', 'LOAO', 'SSO', 'UKIRT', 'MAO']:
			# print(obs)
	...
	# print('THERE IS A ERROR DURING PROCESS.\nSEND MAIL TO STAFFS.')
		...
		f.write('THERE IS NO CANDIDATES.\n')
