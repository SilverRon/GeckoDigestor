...
try:
	...
	cat = Table.read('../data/GLADE+_240919.fits', format='fits',)				
...
while True:
	...
	#	Mock data
	# event_nolog_list = [f"MS240408c_INITIAL"]
	# event_nolog_list = [f"S240422ed_UPDATE"]
	# %%
		...
		for ee, event in enumerate(event_nolog_list):
			...
			if os.path.exists(path_record):
				...
				if skymap_str:
					...
					skymap.meta['DISTMEAN'] = skymap.meta.get('DISTMEAN', 1000)
					skymap.meta['DISTSTD'] = skymap.meta.get('DISTSTD', 100)
					if skymap.meta['DISTMEAN'] == 1000:
						print(f"No Distance Information!")
					#
				...
				if gecko_digestor_trigger:
					...
					if is_condition_satisfied(record):
						...
						# for obs in ['7DT', 'KMTNet',]:
						for obs in ['7DT', 'KMTNet', 'CBNUO', 'RASA36', 'RASA36',]:
						# for obs in ['7DT',]:
						# for obs in ['KCT', 'CBNUO', 'RASA36', 'KMTNet',]:
							...
							if len(select_skygrid_cat) > 0:
								...
								prob_values = select_skygrid_cat[probkey]
								valid_mask = ~np.isnan(prob_values)
								valid_probs = prob_values[valid_mask]

								if len(valid_probs) > 0:
									min_prob = np.min(valid_probs)
									select_skygrid_cat[probkey][~valid_mask] = min_prob
								else:
									print(f"[warning] '{probkey}' column in select_skygrid_cat has no valid entries.")
									
									select_skygrid_cat[probkey] = np.zeros(len(select_skygrid_cat))
								...
								select_skygrid_cat.meta = skygrid_meta_dict
								select_skygrid_cat.write(f"{select_skygrid_cat_name}", format='csv', overwrite=True)
								#------------------------------------------------------------
								# %%
								#	Script Maker
								#------------------------------------------------------------
								# _select_100_skygrid_cat = select_skygrid_cat[:100]
								if obs == '7DT':
									scheduler_grid = ObsScheduler(
										# target_db= _select_100_skygrid_cat,
									...
									# Action
									scriptmaker_grid.write_rts(
										filename_prefix= filename_prefix, 
										savepath = rts_savepath, 
										# n_target_for_each_timeslot= n_target_for_each_timeslot
										#	Hard Coding!
										n_target_for_each_timeslot= 20
										)
									# Temp. Commented out
									# rts_table = scriptmaker_grid.obsdata.schedule.scheduled[scriptmaker_grid.obsdata.schedule.scheduled['obj'] != 'autofocus']
									# rts_filename_for_7DT = f"{rts_savepath}/{filename_prefix}_rts_{obs}.csv"
									# rts_table.write(rts_filename_for_7DT, format = 'csv', overwrite=True)
									# #	Google Spread Sheet
									# gscom = f"python googlesheet.py {event} {rts_filename_for_7DT}"
									# print(gscom)
									# os.system(gscom)
									# print(f"Done!")
									...
									n_observable_7DT = len(scriptmaker_grid.result.scheduled)
									print(f"{n_observable_7DT} 7DT observations")
									print("Done!\n")
								#
								#------------------------------------------------------------
								# %%
								...
								if obs in ['RASA36', 'KCT', 'CBNUO']:
									...
									n_observable_7DT = len(select_skygrid_cat)
									print(f"{n_observable_7DT} {obs} observations")
									print("Done!\n")
						#============================================================
						#	Summary
						#------------------------------------------------------------
						...
						area_50%(deg2) {area_50.to(u.deg**2).value:.1f}
						...
						_summary_txt +=f"""7DT {n_observable_7DT}
						SAO {n_observable_SAO}
